...
#define WIFI_MANAGER_STACK_SIZE      2048
#define WIFI_MANAGER_INIT_PRIORITY   90
#define WIFI_MANAGER_THREAD_PRIORITY 7
...
static int wifi_settings_set(const char *name, size_t len_rd, settings_read_cb read_cb,
			     void *cb_arg)
{
	uint8_t *buffer;
	size_t buffer_len;
...
K_THREAD_STACK_DEFINE(wifi_manager_work_q_stack, WIFI_MANAGER_STACK_SIZE);
...
static int wifi_manager_init(void)
{
	...
	k_work_queue_start(&wifi_mgmt->workq, config->workq_stack, config->workq_stack_size,
			   WIFI_MANAGER_THREAD_PRIORITY, &config->workq_config);
...
SYS_INIT(wifi_manager_init, APPLICATION, WIFI_MANAGER_INIT_PRIORITY);
